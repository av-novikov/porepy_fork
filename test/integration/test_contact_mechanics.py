...
"""
import contact_mechanics_examples
import unittest
import numpy as np
import porepy as pp
...
class TestContactMechanics(unittest.TestCase):
    def _solve(self, setup):
        ...
        phi = 0#np.pi / 10
        coords = np.sqrt((mg.cell_centers[0,:int(mg.num_cells / 2)] - mg.cell_centers[0,0]) ** 2 + (mg.cell_centers[1,:int(mg.num_cells / 2)] - mg.cell_centers[1,0]) ** 2)
        coords += coords[1] / 2

        setup.reconstruct_stress()
        for g, d in setup.gb:
            if g.dim == setup.Nd:
                tr = d['state']['stress'].reshape(int(d['state']['stress'].size / 2), 2)
                areas = g.face_areas[g.face_centers[1, :] == 0.5]
                orig_tr = tr[g.face_centers[1, :] == 0.5, :]


        return coords, u_mortar_local_decomposed, contact_force / areas[::2], orig_tr
    ...
    def test_push_north_zero_opening(self):
        ...
        setup = SetupContactMechanics(
            ux_south=0, uy_south=0, ux_north=0.01, uy_north=-0.001
        )

        coords, u_mortar, contact_force, orig_tr = self._solve(setup)
        ...
        np.savetxt('pp_data.txt', np.c_[coords, u_mortar[0], np.fabs(contact_force[0]), np.fabs(contact_force[1])])
        fig, ax = plt.subplots(ncols=1, sharey=False, figsize=(10,6))
        tr = ax.twinx()
        #ax.plot(coords, u_mortar[0], 'b', label='slip')
        #ax.plot(coords, u_mortar[1], 'b--', label='gap')
        #tr.plot(coords, np.abs(contact_force[0] / np.max(np.abs(contact_force[0]))), 'r', label='traction_t')
        #tr.plot(coords, np.abs(contact_force[1] / np.max(np.abs(contact_force[1]))), 'r--', label='traction_n')
        ax.plot(coords, contact_force[0], 'b', label='traction_t')
        tr.plot(coords, -contact_force[1], 'r', label='traction_n')
        #tr.plot(coords, np.fabs(orig_tr[:40,1]), 'r--', label='orig_traction_t')
        #tr.plot(coords, np.fabs(orig_tr[40:,1]), 'r--', label='orig_traction_t')
        #tr.plot(coords, np.fabs(contact_force[1]), 'r--', label='traction_n')
        ax.grid(True)
        ax.set_xlabel('distance')
        ax.set_ylabel('tangential traction')
        tr.set_ylabel('normal traction')
...
class SetupContactMechanics(
    contact_mechanics_examples.ContactMechanicsExample
):
