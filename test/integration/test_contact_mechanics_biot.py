...
class TestContactMechanicsBiot(unittest.TestCase):
    def _solve(self, setup):
        ...
        coords += coords[1] / 2
        return coords, u_mortar_local_decomposed, contact_force, fracture_pressure
    ...
    def test_time_dependent_push_north_zero_opening(self):
        ...
        setup.uy_north = -0.001
        setup.ux_north = 0.01
        setup.time_step = 86400.0
        setup.end_time = 30 * 86400.0
        #setup.mesh_args = [2, 2]
        #setup.simplex = False
        # setup.subtract_fracture_pressure = False
        coords, u_mortar, contact_force, fracture_pressure = self._solve(setup)

        np.savetxt('pp_data.txt', np.c_[coords, u_mortar[0], np.fabs(contact_force[0])])
        ...
        tr.plot(coords, np.fabs(contact_force[0]), 'r', label='traction_t')
        #tr.plot(coords, np.fabs(contact_force[1]), 'r--', label='traction_n')
        ax.grid(True)
        ax.set_xlabel('distance')
        ax.set_ylabel('slip')
        tr.set_ylabel('traction')
...
class SetupContactMechanicsBiot(
    contact_mechanics_examples.ProblemDataTime, model.ContactMechanicsBiot
):
    def __init__(self):
    ...
    def create_grid(self) -> None:
        ...
        l = 0.5
        c = np.array([0.5, 0.5])
        x_end = np.array([c[0] - l/2 * np.cos(phi), c[0] + l/2 * np.cos(phi)])
        y_end = np.array([c[0] - l/2 * np.sin(phi), c[0] + l/2 * np.sin(phi)])

        self.frac_pts = np.array([[c[0] - l/2 * np.cos(phi), c[0] - l/2 * np.sin(phi)],
                                  [c[0] + l/2 * np.cos(phi), c[0] + l/2 * np.sin(phi)]])
        ...
        #gb, self.box = gb, self.box = pp.grid_buckets_2d.single(self.mesh_args, x_end, y_end)
